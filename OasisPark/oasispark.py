...
def main():
    ...
    cursor.execute('select idVeiculo, Placa, Cor, Modelo, Veiculo.idCliente, idVaga, DataHora_Entrada, DataHora_Saida, Valor, Veiculo.idAtendente, Comprovante, CpfCliente, Plano.nomePlano from Veiculo inner join Plano on Veiculo.idPlano = Plano.idPlano inner join Cliente on Veiculo.idCliente = Cliente.idCliente where DataHora_Saida is null')
    data = cursor.fetchall()
    conn.commit()
